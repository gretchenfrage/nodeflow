			...
			while (shouldContinue) {
				...
				if (transmissionType == SocketConstants.PAYLOAD_MASK) {
					if (socket.isPresent()) {
						...
						in.read(payload);
						ReceivedPayload received = new ReceivedPayload(payloadID, messageID, partNumber, totalParts,
								payload);
					...
					if (socket.isPresent()) {
						...
						byte[] payload = new byte[payloadSize];
						in.read(payload);
						ReceivedPayload received = new ReceivedPayload(payloadID, messageID, ordinal, partNumber,
								totalParts, payload);
						socket.get().receivePayload(received);
					}
					...
					System.err.println("Invalid transmission type: " + Long.toBinaryString(transmissionType));
					System.out.println("payload mask: " + Long.toBinaryString(SocketConstants.PAYLOAD_MASK));
					System.out.println("ordered mask: " + Long.toBinaryString(SocketConstants.ORDERED_PAYLOAD_MASK));
					System.out.println("start connection mask: " + Long.toBinaryString(SocketConstants.START_CONNECTION_MASK));
					System.out.println("disconnect connection mask: " + Long.toBinaryString(SocketConstants.DISCONNECT_CONNECTION_MASK));
					System.out.println("accept connection mask: " + Long.toBinaryString(SocketConstants.ACCEPT_CONNECTION_MASK));
					System.out.println("reject connection mask: " + Long.toBinaryString(SocketConstants.REJECT_CONNECTION_MASK));
					System.out.println("confirm payload mask: " + Long.toBinaryString(SocketConstants.CONFIRM_PAYLOAD_MASK));
					System.out.println("heartbeat mask: " + Long.toBinaryString(SocketConstants.PAYLOAD_MASK));
				}
			}
		} catch (IOException e) {
		}
